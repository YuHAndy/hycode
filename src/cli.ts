...
/** 用户配置的一个供应商（含 API Key 与多个模型 ID） */
interface ConfiguredProvider {
	id: string;
	name: string;
	baseUrl: string;
	api: ProviderApi;
	authHeader: boolean; // true = Authorization: Bearer；false = x-api-key
	apiKey: string;
	models: string[];
	/** 上下文窗口（tokens），用于上下文占用百分比统计；缺省 128000 */
...
interface HyCodeSettings {
	footer: FooterItem[];
	/** 全屏模式（TuiAltScreen）；默认 false = 终端顺沿（TuiMainScreen） */
	fullscreen?: boolean;
	/** 当前召唤的宠物 id（/buddy 设置；重启自动恢复），无则不显示 */
	buddy?: string;
}

/** 配置文件结构（模型宇宙严格等于用户添加；settings 为 UI 设置） */
interface HyCodeConfig {
	providers: ConfiguredProvider[];
	settings: HyCodeSettings;
}

/** 交互输入适配：standalone（键盘选择/掩码）与 REPL（readline 行输入）共用同一套向导 */
interface PromptAdapter {
	/** 文本输入；用户按 Esc 取消时返回 undefined */
	askText(prompt: string): Promise<string | undefined>;
	/** 掩码输入；用户按 Esc 取消时返回 undefined */
	askSecret(prompt: string): Promise<string | undefined>;
	askChoice(title: string, items: string[]): Promise<number>;
}
...
function renderLogo(): string {
	...
	const padded = lines.map((line, i) => {
		if (i === 0) return `${line}  ${DIM}${ver}${RESET}${GREEN}`;
...
function isConfiguredProvider(value: unknown): value is ConfiguredProvider {
	const p = value as Partial<ConfiguredProvider> | undefined;
...
function loadConfig(): HyCodeConfig {
	try {
		if (existsSync(CONFIG_PATH)) {
			const raw = readFileSync(CONFIG_PATH, "utf8");
			const cfg = JSON.parse(raw) as HyCodeConfig;
			if (cfg && Array.isArray(cfg.providers)) {
				...
				for (const p of providers) {
					if (p.contextWindow === 128000) {
						...
						if (cat !== undefined && cat !== 128000) p.contextWindow = cat;
					}
				}
...
function normalizeSettings(settings: Partial<HyCodeSettings> | undefined): HyCodeSettings {
	const valid = new Set(FOOTER_ITEMS.map((it) => it.key));
	const footer = Array.isArray(settings?.footer)
		? (settings.footer.filter((k): k is FooterItem => valid.has(k as FooterItem)) as FooterItem[])
		: [...DEFAULT_FOOTER];
	return { footer: footer.length > 0 ? footer : [...DEFAULT_FOOTER], fullscreen: settings?.fullscreen === true };
}

function saveConfig(config: HyCodeConfig): void {
	mkdirSync(dirname(CONFIG_PATH), { recursive: true });
	writeFileSync(CONFIG_PATH, JSON.stringify(config, null, "\t") + "\n", "utf8");
}

// ---------------------------------------------------------------------------
// Interactive input primitives（键盘导航 + 掩码，用于 standalone 向导）
// ---------------------------------------------------------------------------

type KeypressHandler = (str: string | undefined, key: { name: string; ctrl?: boolean }) => void;

function initKeypress(stdin: NodeJS.ReadStream): void {
	const s = stdin as unknown as { _hycodeKeypress?: boolean };
	if (s._hycodeKeypress) return;
...
function waitKey(stdin: NodeJS.ReadStream, stdout: NodeJS.WriteStream, handler: KeypressHandler): () => void {
	const onKey = (str: string | undefined, key: { name: string; ctrl?: boolean }) => handler(str, key);
...
function createKeypressAdapter(stdin: NodeJS.ReadStream, stdout: NodeJS.WriteStream): PromptAdapter {
	return {
...
function createTuiPromptAdapter(
	...
	const askSecret = (prompt: string): Promise<string | undefined> =>
		new Promise((resolve) => {
	...
	const askChoice = (title: string, items: string[]): Promise<number> =>
		new Promise((resolve) => {
...
class PromptDialog implements Component {
	private readonly prompt: string;
	private readonly input: Input;
	public onCancel?: () => void;

	constructor(prompt: string, input: Input) {
		this.prompt = prompt;
		this.input = input;
	}
...
/** 截断工具详情（面板宽度有限） */
function truncateDetail(detail: string, max = 48): string {
 ...
 * 每用户回合重置：显示当前回合的思考过程。
 */
class ThinkingEntry implements Component {
	private text = "";
	...
	render(width: number): string[] {
		const head = "⚛ 思考";
		if (this.collapsed) {
			if (!this.hasContent) return [head]; // 无内容也占一行，保持布局稳定
 ...
 */
class ToolEntry implements Component {
	private records: Array<{ id: string; base: string; done: boolean; error: boolean }> = [];
	private collapsed = true;

	/** 记录一条工具调用开始（icon + 名称 + 具体操作，如 🖥️ bash: ls -la） */
	start(id: string, base: string): void {
		this.records.push({ id, base, done: false, error: false });
		if (this.records.length > MAX_TOOL_ENTRIES) this.records.shift();
	}
	...
	private line(rec: { base: string; done: boolean; error: boolean }): string {
		const mark = rec.done ? (rec.error ? `${RED}✗${RESET}` : `${GREEN}✓${RESET}`) : "";
		return `${rec.base} ${mark}`.trimEnd();
	}

	render(width: number): string[] {
		if (!this.hasRecords) return [`${DIM}🛠️ 工具${RESET}`]; // 无内容也占一行，保持布局稳定
		if (this.collapsed) {
...
/** 一种宠物的外观定义：多帧 ASCII 动画（每帧等宽、半角字符） */
interface BuddySpec {
	id: string;
	name: string; // 显示名（如 "猫咪"）
	emoji: string; // 名字行前缀
	frames: string[][]; // 动画帧，每帧是一个等宽行数组
	interval: number; // 帧间隔 ms
	/** 帧内用点缀色的字符（如眼睛 o、尾巴 ~）；其余可见字符用身体色 */
	accentChars?: string;
	/** 身体色（默认橙色，可覆盖以区分不同宠物） */
	bodyColor?: string;
	/** 点缀色（默认浅绿） */
...
const BUDDIES: BuddySpec[] = [BUDDY_CAT, BUDDY_PUG];

/** 给一帧行上色：点缀字符用点缀色，其余可见字符用身体色，空格/下划线保持原样 */
function colorBuddyFrame(spec: BuddySpec, frame: string[]): string[] {
	return frame.map((line) => {
		let out = "";
		for (const ch of line) {
			if (ch === " " || ch === "_") out += ch;
			else if (spec.accentChars?.includes(ch)) out += (spec.accentColor ?? BUDDY_ACCENT) + ch + RESET;
			else out += (spec.bodyColor ?? BUDDY_BODY) + ch + RESET;
		}
		return out;
	});
}
...
function padRightToWidth(text: string, total: number): string {
	const w = visibleWidth(text);
	return w >= total ? text : text + " ".repeat(total - w);
}

/** TUI 宠物组件：按帧间隔轮播动画，作为 overlay 显示在终端角落 */
class BuddyPet implements Component {
	readonly spec: BuddySpec;
	private frameIdx = 0;
	private timer: NodeJS.Timeout | undefined;
	private onFrame: () => void;

	constructor(spec: BuddySpec, onFrame: () => void) {
		this.spec = spec;
...
/** 国产常用 LLM 供应商预设（官方地址 + 模型 ID，用户只需填 API Key） */
...
const MODEL_WINDOW_CATALOG: ReadonlyArray<readonly [string, number]> = [	["deepseek-chat", 1_000_000],
...
/** 按模型 ID 匹配目录中的上下文窗口 */
function catalogWindow(models: string[]): number | undefined {
	for (const m of models) {
		for (const [id, w] of MODEL_WINDOW_CATALOG) {
			if (m.includes(id)) return w;
		}
	}
	return undefined;
}
...
async function collectProvider(ask: PromptAdapter): Promise<ConfiguredProvider | undefined> {
	// ---- 步骤 1：选择供应商（内置国产常用）----
	...
	if (choice < 0) return undefined; // Esc 取消
	...
	if (choice < PROVIDER_PRESETS.length) {
		...
		if (apiKey === undefined) return undefined; // Esc 取消
		if (!apiKey) {
		...
		const modelsInput = await ask.askText(
			`[第 3/${total} 步] 模型 ID（支持多个，逗号分隔；请填当前可用 ID，如 ${preset.models[0]}）: `,
		);
		if (modelsInput === undefined) return undefined; // Esc 取消
	...
	if (id === undefined || !id) return undefined; // Esc 或留空取消
	...
	if (baseUrl === undefined) return undefined; // Esc 取消
	if (!baseUrl) {
	...
	const contextWindowRaw = (
		await ask.askText(`[第 9/${total} 步] 上下文窗口 tokens（回车默认 ${catalogDefault >= 1000 ? `${Math.round(catalogDefault / 1000)}k` : catalogDefault}）: `)
	)?.trim();
	if (contextWindowRaw === undefined) return undefined; // Esc 取消
	const contextWindow = contextWindowRaw ? Number(contextWindowRaw) : catalogDefault;
	if (contextWindowRaw && (!Number.isInteger(contextWindow) || (contextWindow as number) <= 0)) {
...
function upsertProvider(config: HyCodeConfig, p: ConfiguredProvider): { added: string[]; isNew: boolean } {
	...
	if (existing) {
		...
		if (p.contextWindow) existing.contextWindow = p.contextWindow;
...
function removeTarget(config: HyCodeConfig, target: string): string | undefined {
	...
	if (modelPart) {
		...
		if (prov.models.length === 0) config.providers.splice(idx, 1);
...
function countModels(config: HyCodeConfig): number {
	return config.providers.reduce((n, p) => n + p.models.length, 0);
}

/** standalone 添加向导：hycode --add-model */
async function runStandaloneAddModel(config: HyCodeConfig): Promise<number> {
...
function registerProvider(runtime: ModelRuntime, p: ConfiguredProvider): void {
...
function registerAllProviders(runtime: ModelRuntime, config: HyCodeConfig): void {
	for (const p of config.providers) registerProvider(runtime, p);
}

/** 供应商被修改/删除后同步注册表：存在则重注册，否则注销。 */
function syncProvider(runtime: ModelRuntime, config: HyCodeConfig, providerId: string): void {
	const p = config.providers.find((x) => x.id === providerId);
	if (p) registerProvider(runtime, p);
	else runtime.unregisterProvider(providerId);
}
...
function parseArgs(argv: string[]): CliOptions {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		switch (arg) {
			...
			case "--model": {
				const v = next();
				if (!v) throw new Error(`${arg} 需要一个参数`);
				opts.model = v;
				break;
			}
			case "-t":
			...
			case "--tools": {
				const v = next();
				if (!v) throw new Error("--tools 需要一个参数");
...
function listConfiguredModels(config: HyCodeConfig): void {
...
class InMemoryCredentialStore {
	...
	async list(): Promise<Array<{ providerId: string; type: "api_key" }>> {
		return [...this.credentials.keys()].map((providerId) => ({ providerId, type: "api_key" as const }));
	}
 ...
 */
function ensureAppendSystemPrompt(): void {
 ...
 */
function subscribeStreaming(session: AgentSession, singleLine: boolean): void {
	session.subscribe((event) => {
		switch (event.type) {
			...
			case "tool_execution_start":
				if (singleLine) process.stdout.write(`${DIM} [⟳ ${event.toolName}]${RESET}`);
				break;
			case "tool_execution_end":
				if (singleLine) process.stdout.write(`${DIM}${event.isError ? " ✗" : " ✓"}${RESET}`);
				break;
		}
	});
}
 ...
 */
function reportError(session: AgentSession): void {
...
function getErrorString(session: AgentSession): string | undefined {
	const state = session.state as unknown as {
		errorMessage?: string;
		messages?: Array<{ role?: string; stopReason?: string; errorMessage?: string }>;
	};
...
function getLastUsage(session: AgentSession): { input: number; cacheRead: number; cacheWrite: number } | undefined {
...
/** @文件 引用 → 内容块 */
function buildFileContext(absPath: string): string {
	let content = "";
	try {
...
function buildTree(root: string, maxEntries: number): string {
	const lines: string[] = [];
	let count = 0;
	...
	walk(root, 0);
	if (count >= maxEntries) lines.push(`…（共 ${count}+ 项，已截断）`);
	return lines.join("\n");
}

/** @目录 引用 → 目录树块 */
function buildFolderContext(absPath: string): string {
 ...
 *  - 未匹配的保留原样
 */
function expandReferences(text: string, cwd: string): string {
	return text.replace(/@([^\s@]+)/g, (match, p: string) => {
		const token = p.trim();
		...
		try {
			if (existsSync(abs) && statSync(abs).isFile()) return buildFileContext(abs);
			if (existsSync(abs) && statSync(abs).isDirectory()) return buildFolderContext(abs);
		} catch {
			/* 路径不可读则保留原样 */
		}
		return match;
...
async function runRepl(
	...
	config: HyCodeConfig,
	initialSelected: { provider: string; id: string } | undefined,
	...
	let mode: ModeName = initialMode;
	const applyMode = (m: ModeName): void => {
		if (m === "ask") session.setActiveToolsByName([]);
		else if (m === "plan") session.setActiveToolsByName(PLAN_TOOLS);
		else session.setActiveToolsByName(initialToolNames);
		mode = m;
	};
	...
	const updateStatus = (): void => {
		...
		if (footer.includes("context")) {
			...
			if (window > 0) {
				...
				if (usage) {
					...
					if (cu && cu.percent != null) pct = cu.percent;
				}
				// 与 DSH 一致：整数百分比、上限 100
				if (pct !== undefined) parts.push(`上下文:${Math.min(100, Math.round(pct))}%`);
			}
		}
		...
		if (footer.includes("cwd")) parts.push(`目录:${basename(cwd)}`);
	...
	let streamTarget: { md: Markdown; loader: Loader | undefined; text: string } | undefined;
	...
	const onSessionEvent = (event: Parameters<NonNullable<Parameters<AgentSession["subscribe"]>[0]>>[0]): void => {
		switch (event.type) {
			case "message_start":
	...
	// ---- 会话替换（/new /resume） ----
	const replaceSession = async (next: AgentSession): Promise<void> => {
		unsubscribe();
	...
	let pendingAttachment: string | undefined;

	const finalizePrompt = (prompt: string): string => {
		let out = prompt;
		if (pendingAttachment) {
			const ctx = buildFileContext(pendingAttachment);
			if (ctx) out = `（请基于文件 ${pendingAttachment} 回答/工作）\n${ctx}\n${out}`;
	...
	const runPrompt = async (prompt: string): Promise<void> => {
		...
		try {
			...
			if (errMsg && streamTarget) streamTarget.text += `\n\n[请求失败] ${errMsg}`;
		} catch (err) {
			if (streamTarget) streamTarget.text += `\n\n[错误] ${(err as Error).message ?? err}`;
			else say(`[错误] ${(err as Error).message ?? err}`);
		} finally {
	...
	const attachFile = (absPath: string): void => {
		pendingAttachment = absPath;
		say(`已附加文件: ${basename(absPath)}。请输入你的问题，将基于该文件回答。`);
		tui.requestRender();
	};

	const browseDirectory = (dir: string): void => {
	...
	const handleAtReference = (rest: string): void => {
		const token = rest.trim();
		if (token === "workspace" || token === ".") return; // 交给 expandReferences 展开
		const abs = resolve(cwd, token);
		try {
		...
		if (!token) browseDirectory(cwd); // 裸 @
	};

	// 输入 ? 即时弹出快捷键面板（non-capturing，编辑器保持焦点；取消/继续输入自动消失）
	let shortcutOverlay: OverlayHandle | undefined;
		...
		if (/^@[^\s]*$/.test(text)) {
			if (!atBrowserOpen && !atBrowserDismissed) {
				...
				if (!token) {
					...
					const abs = resolve(cwd, token);
					try {
						if (existsSync(abs) && statSync(abs).isDirectory()) browseDirectory(abs);
					} catch {
						/* 忽略不可读路径 */
					}
				}
		...
		for (const pid of providers) {
			...
			if (m) {
				try {
					...
					say(`[错误] 切换失败: ${(err as Error).message ?? err}`);
					return;
				}
	...
	let buddyPet: BuddyPet | undefined;
	let buddyOverlay: OverlayHandle | undefined;

	/** 召唤指定宠物（先移除旧的；保存配置以便重启恢复） */
	const spawnBuddy = (spec: BuddySpec): void => {
		if (buddyOverlay) {
			buddyOverlay.hide();
	...
	const handleBuddyCommand = (arg: string): void => {
	...
	const selectModelInteractive = (): void => {
		const items: string[] = [];
		for (const p of config.providers) {
			for (const m of p.models) items.push(`${p.id}/${m}`);
		}
		if (items.length === 0) {
		...
		const list = new SelectList(
			items.map((v) => ({ value: v, label: v, description: v === current ? "当前" : undefined })),
	...
	const addModels = async (): Promise<void> => {
		let keep = true;
		while (keep) {
	...
	const removeModels = async (target: string): Promise<void> => {
	...
	const resetModels = async (): Promise<void> => {
		...
		config.providers = [];
		saveConfig(config);
		for (const id of removedIds) session.modelRuntime.unregisterProvider(id);
		selected = undefined;
		say("已清空所有模型。");
	...
	const editSettings = (): void => {
		const items: SettingItem[] = FOOTER_ITEMS.map((it) => ({
			id: it.key,
			label: it.label,
		...
		const list = new SettingsList(
			...
			settingsListTheme,
			(id, newValue) => {
				const key = id as FooterItem;
				const set = new Set(config.settings.footer);
				if (newValue === "显示") set.add(key);
				else set.delete(key);
	...
	const handleResume = async (): Promise<void> => {
		...
		const list = new SelectList(
			files.map((f) => ({ value: f, label: basename(f) })),
		...
		list.onSelect = async (item) => {
			...
			try {
				...
				say(`[错误] 恢复失败: ${(err as Error).message ?? err}`);
			}
		};
		list.onCancel = () => {
			overlay.hide();
	...
	const handleCompact = async (): Promise<void> => {
		...
		try {
			...
			say(`[错误] 压缩失败: ${(err as Error).message ?? err}`);
		}
	};
	...
	/** !命令 / !!命令：本地执行 */
	const runBash = (cmd: string, sendToModel: boolean): void => {
	...
	const handleCommand = async (cmd: string): Promise<boolean | "exit"> => {
	...
	// ---- 提交处理 ----
	editor.onSubmit = async (value: string): Promise<void> => {
		...
		if (trimmed.startsWith("/")) {
			try {
				...
				say(`[错误] ${(err as Error).message ?? err}`);
			}
			return;
		}
	...
	const farewell = (): void => {
		if (buddyPet) buddyPet.stop(); // 停掉宠物动画定时器
		session.dispose();
	...
	if (savedBuddy) spawnBuddy(savedBuddy);
...
async function runOnce(session: AgentSession, prompt: string): Promise<number> {
...
async function main(): Promise<number> {
	...
	let initialSelected: { provider: string; id: string } | undefined;
	...
	const initialMode: ModeName = opts.noTools ? "ask" : opts.readonlyTools ? "plan" : "auto";
...
function firstModel(config: HyCodeConfig): { provider: string; id: string } | undefined {
...
main().then(
	...
	(err) => {
		console.error(`[错误] ${(err as Error).message ?? err}`);
		process.exit(1);
	},
);
